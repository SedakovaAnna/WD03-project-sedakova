...
//загруженное изображение
		if ( isset($_FILES['avatar']['name']) && $_FILES['avatar']['tmp_name'] != "" ) {
			...
			$fileTmpLoc = $_FILES["avatar"]["tmp_name"];//временное хранилище файла
			$fileType = $_FILES["avatar"]["type"];//тип файла
			$fileSize = $_FILES["avatar"]["size"];//размер
			$fileErrorMsg = $_FILES["avatar"]["error"];
			$kaboom = explode(".", $fileName);//разбить строку по .
			$fileExt = end($kaboom);//расширение

			// Check file propertties on different conditions
			list($width, $height) = getimagesize($fileTmpLoc);//создать переменные из массива
